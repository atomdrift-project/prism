...
if (millerEl && treeDataEl && detail) {
  ...
  function renderColumn(parent, depth) {
    ...
    for (const child of children) {
      ...
      // Each row contributes to the tree's keyboard navigation. Only
      // one row across the whole tree carries tabindex=0 at a time
      // (roving tabindex); the rest are reachable by arrow keys.
      li.setAttribute("tabindex", "-1");
      li.setAttribute("aria-level", String(depth + 1));
      if (child.d) li.setAttribute("aria-expanded", next && child === next ? "true" : "false");
      const isSelectedDir = next && child === next;
      const isSelectedFile = !child.d && state.selectedSha && child.sha === state.selectedSha;
      if (!child.d) li.classList.add("leaf");
      if (isSelectedDir || isSelectedFile) {
      ...
      // pair it with a visually-hidden word so SR users hear it too.
      const riskWord = classForRisk(child);
  ...
  function renderHintColumn() {
    ...
    p.textContent =
      "Folders show their worst-ranked child. Click a file to see its traits, strings, symbols, and metadata.";
    hint.appendChild(p);
  ...
  function render() {
    ...
    // Roving-tabindex: pick the single row that's keyboard-reachable via
    // Tab. We prefer the selected row in the rightmost column; if there
    // isn't one yet, the first row of the rightmost column.
    setRovingFocus();
  }

  function setRovingFocus() {
    const cols = Array.from(millerEl.querySelectorAll(".files-miller-col"));
    if (!cols.length) return;
    // Prefer the rightmost column that actually contains rows (the
    // hint column has none, so we fall back leftward if needed).
    let target = null;
    for (let i = cols.length - 1; i >= 0 && !target; i--) {
      target =
        cols[i].querySelector(".files-miller-row.selected") ||
        cols[i].querySelector(".files-miller-row");
    }
    if (!target) return;
    for (const r of millerEl.querySelectorAll(".files-miller-row")) {
  ...
  function showSection(sha, section) {
    ...
    if (header) {
      // Sync subtab visual + ARIA state. We keep roving tabindex on the
      // subtab list so arrow keys move focus between subtabs while only
      // one is in the natural Tab order at a time.
      for (const btn of header.querySelectorAll(".files-subtab")) {
        const selected = btn.dataset.sub === section;
  ...
  function scrollToLine(target, flashLines) {
    ...
    if (anchor)
      anchor.scrollIntoView({
        block: "center",
        behavior: smoothScrollAllowed() ? "smooth" : "auto",
      });
  }
  ...
  async function maybeRenderSource(sha) {
    ...
    if (data.tooLarge) {
      sourceEl.innerHTML =
        '<div class="fs-source-placeholder">File is ' +
        (data.sizeStr || "large") +
        " — too large to display inline. Use the ↗ link to open it in its own page.</div>";
      clearTopTraits(sha);
    } else {
      ...
      announce(
        "Source loaded. " +
          currentTraitIndex.size +
          " trait" +
          (currentTraitIndex.size === 1 ? "" : "s") +
          " fired."
      );
  ...
  // Subtab keyboard handling: Left/Right arrows rove focus between
  // visible subtabs and auto-activate the focused one. Hidden subtabs
  // (e.g. Source before the file's text loads) are skipped.
  detail.addEventListener("keydown", (ev) => {
    const btn = ev.target.closest(".files-subtab");
    if (!btn) return;
    const list = btn.closest(".files-detail-subtabs");
    if (!list) return;
    const subs = Array.from(list.querySelectorAll(".files-subtab:not([hidden])"));
    const i = subs.indexOf(btn);
    if (i < 0) return;
    let n = -1;
    switch (ev.key) {
      case "ArrowRight":
    ...
    ev.preventDefault();
    const target = subs[n];
    const headerBlock = target.closest(".files-detail-header-block");
    if (headerBlock) showSection(headerBlock.dataset.fileSha, target.dataset.sub);
    target.focus();
  });
  ...
  //   Up/Down   — move focus within the current column
  //   Right     — on a directory, drill in and focus first child
  //   Left      — focus the row in the parent column whose drill-in
  //               produced this column (i.e. step back up the tree)
  //   Enter/Space — activate (select a file or drill a directory)
  //   Home/End  — first/last row in the current column
  millerEl.addEventListener("keydown", (ev) => {
    const row = ev.target.closest(".files-miller-row");
    if (!row) return;
    const list = row.parentElement;
    if (!list) return;
    const rows = Array.from(list.querySelectorAll(":scope > .files-miller-row"));
    ...
    switch (ev.key) {
      ...
      case "End":
        ...
        break;
      case "ArrowRight": {
        // If this row is a directory, drill in. The click handler
        // re-renders the tree; we then focus the new rightmost
        // column's first row.
        if (row.dataset.fileSha) return; // leaf: nothing to enter
        ev.preventDefault();
        row.click();
      ...
      case "ArrowLeft": {
        // Focus the selected row in the column to the left of ours.
        ev.preventDefault();
        const col = row.closest(".files-miller-col");
        const cols = Array.from(millerEl.querySelectorAll(".files-miller-col"));
        const ci = cols.indexOf(col);
        if (ci > 0) {
          const prev = cols[ci - 1];
          const target =
            prev.querySelector(".files-miller-row.selected") ||
            prev.querySelector(".files-miller-row");
  ...
  // Visually-hidden live region for status announcements. We update its
  // text from places where SR users need feedback that's otherwise only
  // visible (source loaded, clipboard copy, etc.).
  const liveEl = document.getElementById("a11y-live");
  function announce(msg) {
    if (!liveEl) return;
    // Clearing first forces SR to re-announce identical strings.
    liveEl.textContent = "";
    requestAnimationFrame(() => {
  ...
  if (search) {
    search.addEventListener("input", () => {
      ...
      for (const m of matches) {
        ...
        li.setAttribute("role", "treeitem");
        li.setAttribute("tabindex", "-1");
        const sel = state.selectedSha === m.sha;
